...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!($Nome_Utilizador and $Email and $Senha)) {
    ...
    if ($nome_utilizador_len < 3) tamanho();

    include "adicionalmente/config.php";

    // Verifique se existe email
    ...
    $stmt->bind_param("sssss", $Nome_Utilizador, $Email, sha1($Senha), $User_Admin, $User_Client);
    $stmt->execute();
    $stmt->close();
...
<body class="w3-light-grey w3-content" style="max-width:500px">
    ...
    <form class="w3-container w3-light-grey" method="post" enctype="multipart/form-data">
        ...
        <p>
            <br>
            <a href="login.php" role="button" class="btn w3-btn botao w3-right btn-rounded">Voltar</a>
